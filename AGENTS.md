...
A request for a message, commands, file selection, PR content, release notes, or a review selects modes 1–3, 5, and 6. A bare "commit this," "go ahead," or approval of proposed commands does not silently switch to mode 4. If execution intent is ambiguous, stay read-only and ask for an explicit autonomous request before mutating git.
...
- **Mode 6 never writes to GitHub, and mode 4 does not extend to it.** Deciding with the user to approve, reject, or merge a pull request produces text they paste; the agent never carries it out. Mode 4 covers staging, committing, and pushing your own work — never reviewing or merging someone else's.
...
## Mode 6: Reviewing an incoming pull request (only when asked)

Runs **only when the user asks for help reviewing or landing a pull request** — "review PR 412", "should I merge this", "look at this PR with me". Written for the maintainer on the receiving end, deciding what to do with someone else's branch. Never fires as part of a commit or PR-authoring request. Read-only throughout: it reads the PR, works the decision through with the user, and produces one Markdown block for them to paste into GitHub.

1. **Gather it, read-only.**
   ...
   Read `CONTRIBUTING.md` if present. Read the existing review threads before forming an opinion — someone may have raised the point already, or the contributor may have answered it. On a fork PR (`isCrossRepository` true), workflows often require maintainer approval before they run, so **empty `gh pr checks` output means "not run," not "passing"** — report which. On a large diff, read `--name-only` first and pull only the files that carry the change. If the base has moved a lot since the PR opened, say so rather than reviewing a stale picture.

2. **Separate blocking from suggestion. Two buckets, only two.** Blocking means merging leaves the project worse off than not merging: breaks the build or an existing test, loses or corrupts data, opens a security hole, breaks a documented contract without notating it, or does not do what the PR claims. Everything else — naming, structure, a nicer approach, broader tests, clearer comments — is a suggestion, and a suggestion is never on its own a reason to withhold a merge. The test: *would you rather have this branch in the tree, or not have it?*

3. **Calibrate the bar to the project.** Small project, few contributors, no stated convention, little CI — the blocking bar is high and most findings are optional. Missing tests in a repo with no test suite is not a blocker; ignoring a convention the project never wrote down is not a blocker. Larger projects with real gates are entitled to enforce them, but a convention miss is still usually a suggestion. Two rules hold at any size: **never manufacture a blocker** to make a review look thorough, and **lead with the verdict** — if nothing blocks, say so in one plain line before anything else.

4. **Talk it through before writing anything.** Present what the PR does, what blocks it if anything, what is merely suggested — then ask what the user wants to do. Say what you would do when asked. Label taste as taste. When they merge over a finding you raised, that is their call; note it once if genuinely risky and do not relitigate.

5. **Produce exactly one block, for the decision they made.** Either a review comment to paste into GitHub's review box, or the merge-commit message if they are accepting. Not both, not preemptively.

6. **Act on nothing.** Never run `gh pr review`, `gh pr comment`, `gh pr merge`, `gh pr close`, `gh pr edit`, or any `gh api` call with a method other than GET. "Approve it", "go ahead", "yes, merge" tell you what the block should say — they are never permission to execute it, and no autonomous request extends here. Confirm the decision, hand over the text, say plainly that they run it themselves, and stop.

### The review comment

Verdict in the first line. Blocking items grouped first, suggestions after, each labelled so the contributor knows what is required versus optional. Point at code and location, not at the person, and say what would resolve it. For an outside contributor the register differs from a teammate's — they gave unpaid work with no context on your standards, so put anything they could not have known on the project, not on them. Note what is genuinely good; skip it rather than manufacturing praise.

### The merge-commit message

Depends on the merge method read in step 1.

- **Squash** — GitHub prefills the subject from the PR title and the body from every branch commit concatenated, which is usually noise, and this is the commit that lands on the default branch and stays in `git log` forever. Write it fresh from the diff with the normal discipline. The contributor's subjects (`wip`, `fix stuff`) are a hint at intent, not source text to tidy.
- **Merge commit** — GitHub's `Merge pull request #<n> from …` default is fine. Leave it unless asked.
- **Rebase** — no new message exists. Say so rather than producing one.

**`Co-authored-by:` is transcribed here, never invented.** A squash collapses the branch into one commit and credits only the PR author, so when `--json commits` shows more than one distinct author, carry each across as `Co-authored-by: Name <email>`, copied exactly from the real commit metadata. This is the one place an attribution trailer is emitted unasked, and it is still not inference — it preserves authorship the squash would otherwise destroy. Add `Closes #<n>` when the PR genuinely links an issue. Nothing else changes: no agent identity, no `Reviewed-by:` unless asked, never a name or address that did not come from a real commit or from the user.
...
Nothing else opts in, with one exception: when squash-merging a pull request in mode 6, `Co-authored-by:` lines are transcribed unasked from the branch's real commit metadata, because the squash collapses every commit into one and would otherwise destroy authorship that already exists. Transcription is not inference — every name and address comes from an actual commit. Otherwise, named humans come only from values the user supplies — never invent a name or an email address, and only add trailers that are true. An invented reviewer or co-author is worse than none.
...
Optional deeper reading — not needed to write a correct commit, PR, or release note: `references/` in this repository holds the full Conventional Commits grammar (`conventional-commits.md`), an annotated example gallery with worked splits and a file-selection walkthrough (`examples.md`), message craft (`craft.md`), scope/convention guidance (`scopes-and-repos.md`), pull-request guidance (`pull-requests.md`), release-note guidance (`release-notes.md`), and guidance for reviewing an incoming pull request (`pr-review.md`).
