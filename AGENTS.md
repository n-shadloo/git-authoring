...
4. **Autonomous stage, commit, and push:** run those operations end to end only when the user unmistakably asks the agent to do the work itself, such as "stage, commit, and push this for me" or "do it all yourself."

A request for a message, commands, file selection, or PR content selects modes 1–3. A bare "commit this," "go ahead," or approval of proposed commands does not silently switch to mode 4. If execution intent is ambiguous, stay read-only and ask for an explicit autonomous request before mutating git.
...
Optional deeper reading — not needed to write a correct commit or PR: `references/` in this repository holds the full Conventional Commits grammar (`conventional-commits.md`), an annotated example gallery with worked splits and a file-selection walkthrough (`examples.md`), message craft (`craft.md`), scope/convention guidance (`scopes-and-repos.md`), and pull-request guidance (`pull-requests.md`).
