...
Writing a pull request that a reviewer can act on: a title that says what the branch does, and a description that answers what changed, why, how it was tested, and what breaks. This runs only when the user asks for PR help — it is never part of a commit request. Everything here is grounded in the branch's real history and diff; nothing is invented.
...
- **No AI attribution.** The title and description read as the developer's own work. Never add a "generated by", an AI co-author, or any signal that an AI wrote the PR — include such a note only if the user explicitly asks.
- **Never open the PR yourself.** Produce the Markdown for the user to paste into the PR form. If they'd rather run it from the terminal, offer the exact command and let them run it:
  ...
  Opening the PR — like staging and committing — happens only on the user's explicit go-ahead.
